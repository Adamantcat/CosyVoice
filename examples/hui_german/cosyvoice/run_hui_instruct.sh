...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  for x in hui_train hui_dev hui_test; do
    mkdir -p data/$x/parquet_instruct
    tools/make_parquet_list_instruct.py --num_utts_per_parquet 1000 \
      --num_processes 10 \
      --src_dir data/$x \
      --des_dir data/$x/parquet_instruct \
      --instruct_file data/templates_hui.json
  done
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  for mode in sft zero_shot; do
    python cosyvoice/bin/inference.py --mode $mode \
      ...
      --prompt_data data/test-clean/parquet_instruct/data.list \
      --prompt_utt2data data/test-clean/parquet_instruct/utt2data.list \
...
export NCCL_P2P_DISABLE="1" # Use only on Madagaskarweihe
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  for model in llm; do
    torchrun --nnodes=1 --nproc_per_node=$num_gpus \
      ...
      --model_dir `pwd`/exp_hui_instruct/cosyvoice/$model/$train_engine \
      --tensorboard_dir `pwd`/tensorboard/cosyvoice_hui_instruct/$model/$train_engine \
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  for model in llm; do
    decode_checkpoint=`pwd`/exp_hui_instruct/cosyvoice/$model/$train_engine/${model}.pt
    echo "do model average and final checkpoint is $decode_checkpoint"
    python cosyvoice/bin/average_model.py \
      --dst_model $decode_checkpoint \
      --src_path `pwd`/exp_hui_instruct/cosyvoice/$model/$train_engine  \
      --num ${average_num} \
      --val_best
  done
fi 

model_dir="/mount/arbeitsdaten/textklang/synthesis/CosyVoice/CosyVoice/examples/hui_german/cosyvoice/exp_hui_instruct_pregen/cosyvoice/llm/torch_ddp"
